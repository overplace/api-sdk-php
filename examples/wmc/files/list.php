...
try {
	...
	// Creating request object for get list of files object collection.
	$request = new \Overplace\Request\Wmc\Files\Lists();
	$request->setIdWmc($wmc->getId());
	output("Calling wmc media list using {$wmc->getTitolo()}...");
	$list = $service->getList($request);

	/**
	 * @var \Overplace\Response\Allegato $allegato
	 */
	foreach ($list as $allegato){
		output("{$allegato->getId()} - {$allegato->getUrl()}");
	}
	...
	while (isset($paginator) && $paginator->hasNextPage()){
		output("Calling wmc files list using {$wmc->getTitolo()} - Page {$paginator->getPage()->getNext()}...");
		...
		/**
		 * Iterate over Allegato Collection
		 * @var \Overplace\Response\Allegato $allegato
		 */
		foreach ($list as $allegato){
			output("{$allegato->getId()} - {$allegato->getUrl()}");
